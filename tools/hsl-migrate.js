#!/usr/bin/env node
// HSL migration for tools/waldo-ds.styles.css + waldo-ui/waldo-shadcn-theme.css.
// Color-preserving (HSL == old hex). Usage: node tools/hsl-migrate.js <repo-root>
const fs = require('fs');
...
s = s.replace(/rgba\((\d+),\s*(\d+),\s*(\d+),\s*([0-9.]+)\)/g, (m,r,g,b,a) => {
  const k = `${r},${g},${b}`;
  if (KEEP_RGBA.includes(k)) return m;
