...
**Opacity / color convention (vanilla layer & prototypes).** Tokens live in the HSL theme (`waldo-shadcn-theme.css`); `waldo-ds.css` is token-free and consumes them via `hsl(var(--token))`. Solid = `hsl(var(--token))`; with opacity = `hsl(var(--token) / a)`. The old `-rgb` companion convention (`rgba(var(--token-rgb), a)`) is **retired** — no `-rgb` tokens anymore. Consumers link the theme FIRST, then `waldo-ds.css`. (The React `.tsx` layer uses Tailwind utilities like `bg-primary/12` — unaffected.)
...
- Adveron's design system + change-review app → **Adveron DS**, in its own repo `~/GitHub/adveron-design-system` (moved out of here 2026-09-24)
...
- cwd inside `~/GitHub/adveron-design-system` → you are **Adveron DS** (read its `CLAUDE.md` first). It is no longer inside this repo.
...
**Before deciding any component or chart is missing**, read `docs/component-index.md` — the flat list of ALL consumables. It has THREE sections (core Components, Brand API components, Charts), mirroring `index.html`'s `#comp-*` anchors (core `waldoGo` + Brand API `brandApiGo`) and `#chart-page-*` sections; a partial scan misses the rest and leads to rebuilding things that already exist (a comp-only scan once produced a duplicate "Mention Card" while Comment Card shipped). The index is GENERATED from `index.html` (`node tools/build-component-index.js`) and drift-checked by the guard — never hand-edit it.

**To build a prototype** (`waldo-labs/…`): read `docs/brand-api-dashboard-doctrine.md` FIRST — it holds the prototype rules (layout, cards, topbar, chart palette, etc.). Link the DS via `<link href="../../waldo-ds.css">`, use `var(--chart-1..12)` for data colors, never hardcode hex, no `:root` redeclaring DS tokens. The doctrine is the source of these rules — don't restate or invent them. To validate a prototype, point `detect.js` straight at it: `node tools/detect.js waldo-labs/<proto>/` — an **explicit** waldo-labs path is always scanned (no flag needed). `waldo-labs/` is skipped only on a BROAD scan (`detect.js .`) to keep the commit guard fast; `--include-labs` forces it everywhere.
