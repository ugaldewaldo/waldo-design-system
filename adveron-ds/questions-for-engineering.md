...
I've taken a snapshot of what ships today (waldo-agentic `bcc587a`). That's the
official starting point, and I'm not trying to undo anything in it. These
questions are only about how we work from here on.

## Questions

1. **Could Adveron use Waldo's components directly?** The idea: Waldo's components
   ...
   - Adveron has its own variants (Button `brand`, `solid`, `white`; Badge `mono`).
   - Some Adveron components are built differently (Switch as a native button,
     RadioGroup wired to react-hook-form).

2. **If Adveron stays separate:** could adveron-web consume its components from
...
3. **Who decides?** Justin, which Adveron UI decisions are yours and should stay
...
4. **Claude sessions.** From now on, can we add a rule to the adveron-web
   `CLAUDE.md`: `components/ui/*` and the theme files are never edited as a side
   effect of unrelated work, and any change to them goes through its own PR?

## What I'm asking for

Answers to 1–4, or 20 minutes to go through them together.
